...
function getDayName(date) {
  ...
  const indexDay = correctDate.getDay();
  const daysWeek = {
    0: 'Sunday',
    1: 'Monday',
    2: 'Tuesday',
...
function getNextFriday(date) {
  ...
  correctDate.setDate(correctDate.getDate() + daysAdd);
  return correctDate;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const firstSunday = ((7 - firstDay + 7) % 7) + 1;
  const count =
    Math.ceil((totalDays - firstSaturday + 1) / 7) +
    Math.ceil((totalDays - firstSunday + 1) / 7);
  return count;
}
...
function getWeekNumberByDate(date) {
  ...
  correctDate.setDate(
    correctDate.getDate() + 3 - ((correctDate.getDay() + 6) % 7)
  );
  const yearStart = new Date(correctDate.getFullYear(), 0, 1);
  return Math.ceil(((correctDate - yearStart) / 86400000 + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  const correctDate = new Date(date);
  correctDate.setDate(13);
  while (correctDate.getDay() !== 5 || correctDate <= date) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (beginDate <= endDate) {
    for (let i = 0; i < countWorkDays && beginDate <= endDate; i += 1) {
    ...
    beginDate.setDate(beginDate.getDate() + countOffDays);
  }
  return result;
}
...
function isLeapYear(date) {
  ...
  const div4 = correctYear % 4 === 0;
  const div100 = correctYear % 100 === 0;
  const div400 = correctYear % 400 === 0;
